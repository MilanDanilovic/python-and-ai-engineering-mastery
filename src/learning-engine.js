...
export function createSession(saved,now=new Date()){
 ...
 return {id:`session-${today}`,date:today,createdAt:new Date(now).toISOString(),status:'active',topics,tasks,minutes:80,stage:'review',stagesDone:[],documentation:{topicId:primary.id,problem:`Use the "${milestoneById[primary.id].guide.reading.title}" documentation section to answer: ${milestoneById[primary.id].interviewQuestion} Include a small example and identify the documented rule that explains its behavior.`,answer:'',reference:'',used:false,aiUsed:false},reflection:{},beforeAI:{},aiConsultedAt:null,architectureId:`architecture-session-${today}`};
}
