...
export function SolutionReveal({code,label='Reference solution',onReveal}){
 ...
 return <div className="solution-reveal"><button className="secondary" aria-expanded={open} onClick={()=>{if(!open)onReveal?.();setOpen(!open);}}>{open?'Hide Solution':'Reveal Solution'}</button>{open&&<CodeEditor value={code} label={label} readOnly/>}</div>;
}
